...
{   
    ...
    public class AnimationTCPClient : NetworkPoseProvider
    {
                ...
                while (true)
                {
                    ...
                    using (NetworkStream stream = socketConnection.GetStream())
                    {
                        ...
                        while ((length = stream.Read(buffer, 0, buffer.Length)) != 0)
                        {
                            ...
                            else if (skeletonDesc == null && serverMessage != "")
                            {
        ...
        public void Update()
        {
            ...
            else if (!initialized && skeletonDesc != null)
            {
                initSkeleton();
            }
        ...
        {
            Debug.Log("process skeleton string" + skeletonString);
            try
            {
                Debug.Log("try to construct skeleton desc");
                skeletonDesc = JsonUtility.FromJson<SkeletonDesc>(skeletonString);

                Debug.Log("construct skeleton desc"+skeletonDesc.jointSequence.ToString());
                buildPoseParameterIndexMap(skeletonDesc.jointSequence);
                Debug.Log("buildPoseParameterIndexMap");
            }
            catch (Exception e)
            {
        ...
        public void initSkeleton()
        {
            if (skeletonDesc != null)
            {
                ...
                createDebugSkeleton(skeletonDesc);
        ...
        protected GameObject createJointVisualization(float scale=1.0f)
        {
            ...
            for (int i = 0; i < mesh.vertices.Length; i++)
            {
                ...
                var newV =  new Vector3(v.x, v.y, v.z) * scale;
