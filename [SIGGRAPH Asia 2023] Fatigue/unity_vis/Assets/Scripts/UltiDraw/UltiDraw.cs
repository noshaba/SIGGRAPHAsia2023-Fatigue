...
public static class UltiDraw {
	...
	public static void DrawGUIFunction1(Vector2 center, Vector2 size, int maxLength, float[] values, float yMin, float yMax, Color background, Color line) {
		DrawGUIRectangle(center, size, background);
		if (values.Length > maxLength && maxLength > 0)	{
			Debug.LogError("value length is bigger than maxLength!");
			return;
		}
		float x = center.x - size.x/2f;
		float y = center.y - size.y/2f;
		...
		if (values.Length < maxLength)
			offset = maxLength - values.Length;
		//float scale = yMax - yMin;
		for(int i=0; i<values.Length-1; i++) {
			DrawGUILine(
					new Vector2(x + ((float)i + offset)/(float)(maxLength-1)*size.x, y + Mathf.Clamp(Normalise(values[i], yMin, yMax, 0f, 1f), 0f, 1f)*size.y),
					new Vector2(x + ((float)(i+1) + offset)/(float)(maxLength-1)*size.x, y + Mathf.Clamp(Normalise(values[i+1], yMin, yMax, 0f, 1f), 0f, 1f)*size.y),
				line
			);
		}
	}
	...
	public static void DrawGUIFunctionsCustomize(Vector2 center, Vector2 size, int maxLength, List<float[]> values, float yMin, float yMax, float thickness, Color background, Color[] lines, float borderWidth, Color borderColor) {
		DrawGUIRectangle(center, size, background);
		DrawGUIRectangleFrame(center, size, borderWidth, borderColor);
		maxLength = Mathf.Max(maxLength, values[0].Length);
		float offset = 1.0f - (float)values[0].Length / (float)maxLength;
		float x = center.x - size.x/2f + offset;
		float y = center.y - size.y/2f;
		//float scale = yMax - yMin;
		...
		for(int k=0; k<values.Count; k++) {
			for(int i=0; i<values[k].Length-1; i++) {
				DrawGUILine(
					new Vector2(x + (float)i/(float)(values[k].Length-1)*(size.x - offset), y + Mathf.Clamp(Normalise(values[k][i], yMin, yMax, 0f, 1f), 0f, 1f)*size.y),
					new Vector2(x + (float)(i+1)/(float)(values[k].Length-1)*(size.x - offset), y + Mathf.Clamp(Normalise(values[k][i+1], yMin, yMax, 0f, 1f), 0f, 1f)*size.y),
					thickness,
					lines[k]
				);
			}
